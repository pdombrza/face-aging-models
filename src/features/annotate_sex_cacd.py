...
def main():
    ...
    female_col = pd.DataFrame.from_dict(
        {"gender": ["F" for _ in range(len(female_names))]}
    )
    ...
    merged_df = pd.merge(
        ...
        right_on=male_names["name"],
        how="left",
    )

    female_names = pd.merge(
        female_names, male_names, how="outer", on="name", indicator="common"
    )
    ...
    merged_df = pd.merge(
        merged_df,
        female_names,
        left_on=merged_df["name_only"],
        right_on=female_names["name"],
        how="left",
    )
    ...
    merged_df = merged_df.drop(
        ["name_only", "key_0", "Unnamed: 0", "name", "name_y", "gender_x", "gender_y"],
        axis=1,
    )
