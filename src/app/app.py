    ...
    if model_type == "FRAN":
        ...
        with torch.no_grad():
            if input_age < 35:
                ...
                out_image_intermediate.append(cyclegan2(input_image_tensor.unsqueeze(0).to(ModelManager.device)))
                out_image_intermediate.append(cyclegan1(input_image_tensor.unsqueeze(0).to(ModelManager.device)))
            elif input_age < 50:
                out_image_intermediate.append(cyclegan2(input_image_tensor.unsqueeze(0).to(ModelManager.device), reverse=True))
                out_image_intermediate.append(input_image_tensor)
...
def run_app():
    ...
    input_gender = gr.Radio(["Male", "Female"], label="Specify your gender", value="Male")
